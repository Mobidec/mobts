...
def run_preprocess_stage_1(
    ...
    df_std = _standardize_input(df_raw, counter_col=counter_col, timestamp_col=timestamp_col, count_col=count_col, out_cols=cfg.cols)
    ...
    df_clean, n_obs_changed_to_nan, n_counters_affected = _remove_measurement_errors_all_network(
        df_std, cols=cfg.cols, cfg=cfg.preprocess, data_is_hourly=data_is_hourly, change_to_daily=change_to_daily
    )
    ...
    df_scored = _calculate_outlier_score(
        df_clean_non_sparse, data_is_hourly=data_is_hourly, change_to_daily=change_to_daily, cols=cfg.cols, stl_cfg=cfg.stl
    )
...
def apply_threshold(
    ...
    return (
        out[[cols.counter, cols.timestamp, cols.count, 'out_score']].copy(),
        n_measurement_errors,
        n_counters_with_measurement_errors,
        thr,
    )
...
class preprocess:
    ...
    def run(
        ...
        try:
            ...
            warnings.warn(
                "The default threshold is estimated based on a specific dataset, set to 20 for daily data, and 45 for hourly data. To optimize the threshold and the quality of outlier detection for measurement errors, plot with 'plot_outliers' function with a desired threshold to observe and change the threshold as needed"
            )
    ...
    def plot_outliers(
        ...
        if data_is_hourly and not change_to_daily:
            _plot_outliers_hourly(
                df_scored=df_scored,
                threshold=threshold,
                counters=counters,
                max_counters=max_counters,
                cols=self.cfg.cols,
                out_cfg=self.cfg.outliers,
                plot_cfg=self.cfg.plot,
            )

        elif (data_is_hourly and change_to_daily) or not data_is_hourly:
            _plot_outliers_daily(
                df_scored=df_scored,
                threshold=threshold,
                counters=counters,
                max_counters=max_counters,
