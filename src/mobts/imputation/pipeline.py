...
from ..utils.formatting import _format_datetime, _add_temporal_columns, _determine_temporal_frequency, _validate_frequency, _drop_sparse_counters
...
class impute:
    ...
    def run(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            out_sm = impute_scaled_median(
                df = out_stl, pivot=pivot, donor_map=donor_map, freq=freq, counters=counters_holes,
                cols=self.cols, donors_cfg=self.donors_cfg, out_cfg=self.out_cfg,
            ...
            out_reg = impute_regression(
                df = out_stl, pivot=pivot, donor_map=donor_map, freq=freq, counters=counters_holes,
                cols=self.cols, donors_cfg=self.donors_cfg, stl_cfg=self.stl_cfg, out_cfg=self.out_cfg,
            )
            ...
            if freq == "daily":
                ...
                used[miss & final.eq(stl).to_numpy() & stl.notna().to_numpy() & ~final.eq(reg).to_numpy() & ~final.eq(sm).to_numpy()] = "STL"
            if freq == "hourly":
